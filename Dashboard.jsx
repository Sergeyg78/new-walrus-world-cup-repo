...
function LiveScoresTab({
  ...
  async function autoResolve() {
    ...
    // Normalize: strip accents, lowercase, apply common aliases
  const ALIASES = {
    'curaçao': 'curacao', 'türkiye': 'turkey',
    'united states': 'usa', 'republic of korea': 'south korea',
    'korea republic': 'south korea', 'ir iran': 'iran',
    "côte d'ivoire": 'ivory coast', "cote d'ivoire": 'ivory coast',
    'bosnia-herzegovina': 'bosnia', 'bosnia and herzegovina': 'bosnia',
    'dr congo': 'congo', 'czechia': 'czech republic',
  }
  const norm = (s) => {
    if (!s) return ''
  ...
  for (const pred of pending) {
      const searchText = norm(pred.prediction + ' ' + pred.match)
      const match = finished.find(m => {
        const homeN = m.homeNorm || norm(m.home)
        const awayN = m.awayNorm || norm(m.away)
        // Both teams must appear in prediction text
        const hasBoth = searchText.includes(homeN) && searchText.includes(awayN)
        if (!hasBoth) return false
        // Match must be finished (date <= today)
